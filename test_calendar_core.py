...
# ── ND-013 : rollover hebdomadaire (sémantique commune avec la macro v6.1) ──
def test_rollover_secondary_absent_neutral_and_flagged():
    """Horizon court + nextweek absent_404 = condition normale de la source :
    aucun COVERAGE_SHORTER_THAN_HORIZON, statut VALID, flag méta posée."""
    src = make_source(feed_status={"thisweek": "ok", "nextweek": "absent_404"})
    p = build(raw=[dict(RAW[0])], source=src)
    assert not any(w.startswith("COVERAGE_SHORTER_THAN_HORIZON") for w in p.quality.warnings)
    assert p.quality.status is QualityStatus.VALID
    assert p.quality.week_rollover_pending is True
    meta = to_legacy_payload(p, NOW)["metadata"]
    assert meta["week_rollover_pending"] is True
    assert "Rollover hebdomadaire" in (meta["coverage_note"] or "")


def test_rollover_secondary_error_also_neutral():
    src = make_source(feed_status={"thisweek": "ok", "nextweek": "error:HTTP_500"})
    p = build(raw=[dict(RAW[0])], source=src)
    assert not any(w.startswith("COVERAGE_SHORTER_THAN_HORIZON") for w in p.quality.warnings)
    assert p.quality.week_rollover_pending is True


def test_nextweek_ok_short_coverage_still_degrades():
    """Dès lors que le secondaire EST intégré, un horizon court redevient une
    troncature anormale : warning + DEGRADED (comportement 2.2.0)."""
    src = make_source(feed_status={"thisweek": "ok", "nextweek": "ok"})
    p = build(raw=[dict(RAW[0])], source=src)
    assert any(w.startswith("COVERAGE_SHORTER_THAN_HORIZON") for w in p.quality.warnings)
    assert p.quality.status is QualityStatus.DEGRADED
    assert p.quality.week_rollover_pending is False


def test_no_feed_status_granularity_keeps_legacy_behavior():
    """Source construite sans feed_status (appelant direct) → v2.2.0 strict."""
    p = build(raw=[dict(RAW[0])])
    assert any(w.startswith("COVERAGE_SHORTER_THAN_HORIZON") for w in p.quality.warnings)
    assert p.quality.status is QualityStatus.DEGRADED
